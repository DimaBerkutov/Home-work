...
function countLetterA(val1) {
    let firstCounter = [];
    for(let i = 0; i < val1.length; i++){
        if(val1[i] == 'a'){
            firstCounter.push(val1[i]);}
    }
    return firstCounter.length;
}
console.log('task1',countLetterA(randomString)); // 4
console.log('task1',countLetterA(user.name + javaScript.html)); // 3
...
function reverseEachWord(val) {
    let sentence = val.split(' ');
    let newSentense = [];
    for(let i = 0; i < sentence.length; i++){
        newSentense.push(sentence[i].split('').reverse().join(''));
    }
    return newSentense.join(' ');
}
...
function reverseEachWord(val1, val2) {
    let sentence = val1.split(' ');
    let newSentense = [];
    if(val2 == true){
        sentence.reverse();
    }
...
console.log('task3_1', reverseEachWord('You don\'t have to do anything special to begin using the DOM. Different browsers have different implementations of the DOM', true));
...
console.log('task3_1', reverseEachWord('The Document Object Model (DOM) is a programming interface for HTML and XML documents', true));
// // stnemucod LMX dna LMTH rof ecafretni gnimmargorp a si )MOD( ledoM tcejbO tnemucoD ehT
//
console.log('task3_1', reverseEachWord('Hi my Name is', false));
// // iH ym emaN si
...
function wordCounter(sentence) {
    ...
    for (let i = 0; i < sentenceArr.length; i++){
        if(sentenceArr[i] in newObj){
            newObj[sentenceArr[i]] = newObj[sentenceArr[i]] + 1;
        }else {
            newObj[sentenceArr[i]] = 1;
        }
    }
    return newObj;
}

console.log('task3_2', wordCounter('Both Java and Java Script is programming and programming OOPBased Language'));
...
//console.log('task3_2', wordCounter('asd qwe asd'));
/*
 {
 asd:2
 qwe:1
...
//console.log('task3_2', wordCounter('url http url www url http'));
...
function createHashTags(listOfCompanys) {
    let newObj = {};
    for (let i = 0; i < listOfCompanys.length; i++) {
        let company = 'company';
        newObj[listOfCompanys[i]['_id']] = listOfCompanys[i].company;
    }
    return newObj;
}

console.log('task4', createHashTags(listOfCompanys));
...
console.log('task super', uniqueElements(notUniqArray)); //1,2,5,10,25,30,0,22,3,
console.log('task super', uniqueElements([1, 1, 2, 3, 3])); // 1,2,3